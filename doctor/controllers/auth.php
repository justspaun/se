...
class AuthController{
    public function register() {
        ...
        $role = htmlspecialchars(isset($data['role']) ? $data['role'] : '');
        ...
        if(empty($role)){
            ...
            echo json_encode($response);
            return;
        } else if(!in_array($role, ['doctor', 'user'])) {
            $response['status'] = 'error';
        ...
        $stmt = $conn->prepare('INSERT INTO users (user_id, name, email, password, role, created_at) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->bind_param('ssssss', $user_id,$name, $email, $hashed_password, $role, $created_at);
