...
class UserController {
    public function set_appointment() {
        ...
        $appointment_time = htmlspecialchars($_GET['appointment_time'] ?? '');
        ...
        if (isset($_GET['appointment_time'])) {
            $appointment_time_raw = urldecode($_GET['appointment_time']); // Decode URL-encoded value
            if (preg_match('/^(2[0-3]|[01]?[0-9]):[0-5][0-9](:[0-5][0-9])?$/', $appointment_time_raw)) {
                $appointment_time = htmlspecialchars($appointment_time_raw); // Sanitize after validation
            } else {
                $response['status'] = 'error';
                $response['message'] = 'Invalid Appointment time cannot be empty';
                echo json_encode($response);
                return;
            }
        } else {
        ...
        // Validate appointment time format (HH:mm:ss)
        if (!$this->isValidDateTime($appointment_time, 'H:i:s')) {
            $response['status'] = 'error';
        ...
        if ($result->num_rows === 0) {
            $stmt->close();
            $response['status'] = 'error';
            $response['message'] = 'Doctor is not available at the requested time';
            echo json_encode($response);
            return;
        }
        $stmt->close();
    ...
    public function appointment() {
        ...
        $stmt = $conn->prepare("SELECT a.appointment_id, a.user_id, a.doctor_id, a.appointment_time, a.created_at, d.name, d.email 
                                FROM appointments a 
